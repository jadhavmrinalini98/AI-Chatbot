...
const ChatBotApp = ({onGoBack,chats,setChats,
    activeChat,setActiveChat,onNewChat}) => {
    const[inputValue, setInputValue] =useState('')
    const[messages,setMessages]=useState(chats[0]?.
        messages || [] )

    useEffect(() =>{
        const activeChatObj = chats.find((chat) => chat.id
    ===activeChat)
    ...
    setChats(updatedChats)
    if(id === activeChat){
        const nextActiveChat = updatedChats.length > 0 ?
        ...
        <div className="chat-window">
            ...
            <div className="chat">
                {messages.map((msg,index) => (
                    <div key ={index} className={msg.type === "prompt"
                        ? "prompt": "response"}>
                    {msg.text} 
